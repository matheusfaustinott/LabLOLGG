...
const getMainAndSecondaryLane = () => {
	...
	const lanesSorted = Object.keys(laneCounts).sort(
		(a, b) => laneCounts[b] - laneCounts[a]
	);
...
const MatchCard = ({ match }) => {
	...
	const gameVersion =
		match.info.gameVersion.split('.')[0] +
		'.' +
		match.info.gameVersion.split('.')[1];
	...
	return (
		...
		>
			...
			>
				<strong sx={{ display: 'flex' }}>
					Level: {participant.champLevel}
				</strong>
				<div style={{ display: 'flex', alignItems: 'center' }}>
					<img
						src={`/lanes/${participant.teamPosition}.png`}
						alt={participant.teamPosition}
							...
							participant[`item${index}`] !== 0 && (
								<img
									...
									src={`https://ddragon.leagueoflegends.com/cdn/${
										versao.value
									}/img/item/${participant[`item${index}`]}.png`}
									alt={`Item ${index}`}
...
const MatchList = ({ version }) => {
	...
	useEffect(() => {
		const imageUrls = matches.value.flatMap((match) => {
			...
			const itemImgs = Array.from({ length: 7 })
				.map((_, index) => {
					return participant[`item${index}`] !== 0
